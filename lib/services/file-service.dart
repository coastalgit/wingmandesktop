...
''';
  ...
  /// Checks if a directory exists and has a .git folder (is a repository)
  static Future<bool> isValidRepository(String directory) async {
    ...
    final gitDir = Directory(path.join(directory, '.git'));
    final pubspecFile = File(path.join(directory, 'pubspec.yaml'));
    final packageJsonFile = File(path.join(directory, 'package.json'));
    final gradleFile = File(path.join(directory, 'build.gradle'));
    final csprojFile = Directory(directory).list()
        .where((entity) => entity is File && entity.path.endsWith('.csproj'))
        .isEmpty;
    
    return await gitDir.exists() || 
           await pubspecFile.exists() || 
           await packageJsonFile.exists() || 
           await gradleFile.exists() || 
           !await csprojFile;
  }
